...
// 2. Query Data dari Database (Disamakan dengan laporan_keuangan_menu.php)
$is_filter_mode = !empty($start_date_esc) || !empty($end_date_esc) || !empty($kios_filter_esc);

if ($is_filter_mode) {
    $query_string = "SELECT 
    tb_menu.nama AS Nama_Menu,SUM(tb_list_order.jumlah) AS Jumlah_Terjual,
    ...
    $query_string = "SELECT
    tb_menu.nama AS Nama_Menu,tb_list_order.jumlah AS Jumlah_Terjual,
    tb_order.nama_kios AS Nama_Toko,
...
// Tentukan Nama Kolom Terakhir (M)
$last_col = 'M';
...
$sheet->setCellValue('C' . $header_row, 'Jumlah');
$sheet->setCellValue('D' . $header_row, 'Nama Toko');
$sheet->setCellValue('E' . $header_row, 'Harga Jual');
$sheet->setCellValue('F' . $header_row, 'PPN');
$sheet->setCellValue('G' . $header_row, 'Harga Pembeli');
$sheet->setCellValue('H' . $header_row, 'Total Menu');
$sheet->setCellValue('I' . $header_row, 'Total PPN');
$sheet->setCellValue('J' . $header_row, 'Total Pembeli');
$sheet->setCellValue('K' . $header_row, 'Untung Toko');
$sheet->setCellValue('L' . $header_row, 'Untung RS');
$sheet->setCellValue('M' . $header_row, 'Untung RS + Pajak');
...
foreach ($data as $row) {
    ...
    $sheet->setCellValue('B' . $rowNum, $row['Nama_Menu']);
    $sheet->setCellValue('C' . $rowNum, $row['Jumlah_Terjual'] ?? 0);
    $sheet->setCellValue('D' . $rowNum, $row['Nama_Toko']);
    $sheet->setCellValue('E' . $rowNum, $row['Harga_Jual_Per_Menu'] ?? 0);
    $sheet->setCellValue('F' . $rowNum, $row['Harga_PPN'] ?? 0);
    $sheet->setCellValue('G' . $rowNum, $row['Harga_Pembeli_Per_Menu'] ?? 0);
    $sheet->setCellValue('H' . $rowNum, $row['Harga_Total_Per_Menu'] ?? 0);
    $sheet->setCellValue('I' . $rowNum, $row['Harga_Total_PPN'] ?? 0);
    $sheet->setCellValue('J' . $rowNum, $row['Harga_Pembeli_Total'] ?? 0);
    $sheet->setCellValue('K' . $rowNum, $row['Keuntungan_Toko'] ?? 0);
    $sheet->setCellValue('L' . $rowNum, $row['Keuntungan_RS'] ?? 0);
    $sheet->setCellValue('M' . $rowNum, $row['Keuntungan_RS_Pajak'] ?? 0);
    
    // Terapkan border dan alignment untuk baris data
    $sheet->getStyle('A' . $rowNum . ':' . $last_col . $rowNum)->applyFromArray($dataStyle);

    // Format mata uang (Kolom E sampai M)
    ...
    // Rata kiri untuk teks (Kolom B, D)
...
$sheet->setCellValue('H' . $rowNum, $total_harga_total);
$sheet->setCellValue('I' . $rowNum, $total_ppn);
$sheet->setCellValue('J' . $rowNum, $total_pembeli_total);
$sheet->setCellValue('K' . $rowNum, $total_keuntungan_toko);
$sheet->setCellValue('L' . $rowNum, $total_keuntungan_rs);
$sheet->setCellValue('M' . $rowNum, $total_keuntungan_rs_pajak);
...
// Gabungkan kolom E sampai M untuk kolom Grand Total
