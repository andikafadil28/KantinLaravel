...
$sheet->setTitle('Laporan Keuntungan Food Court');

// Tentukan Nama Kolom Terakhir (G)
$last_col = 'G';
...
$sheet->setCellValue('A1', 'LAPORAN PENDAPATAN SAKINA FOOD COURT');
...
$headerStyle = [
    ...
    'fill' => [
        ...
        'startColor' => ['rgb' => '17A2B8'], // Background Biru Muda/Cyan untuk RS
    ],
    'alignment' => [
...
$sheet->setCellValue('E' . $header_row, 'Pendapatan Sakina Food Court');
$sheet->setCellValue('F' . $header_row, 'Waktu Order');
$sheet->setCellValue('G' . $header_row, 'Nama Toko');
...
foreach ($data as $row) {
    ...
    $sheet->setCellValue('E' . $rowNum, $row['nominal_rs']);
    $sheet->setCellValue('F' . $rowNum, $row['waktu_order']);
    $sheet->setCellValue('G' . $rowNum, $row['nama_kios']);
    
    // Terapkan border dan alignment untuk baris data
    $sheet->getStyle('A' . $rowNum . ':' . $last_col . $rowNum)->applyFromArray($dataStyle);

    // Format mata uang (Kolom E)
    $sheet->getStyle('E' . $rowNum)->getNumberFormat()->setFormatCode('"Rp "#,##0');
    
    // Rata kiri untuk teks (Kolom C dan G)
...
$totalRowStyle = [
    ...
    'fill' => [
        'fillType' => Fill::FILL_SOLID,
        'startColor' => ['rgb' => 'D4EDDA'], // Background Hijau Muda
    ],
    'borders' => [
...
// Gabungkan kolom F dan G agar sejajar dengan kolom total
...
$filename = "laporan-keuntungan-food-court-" . strtolower($nama_toko_file) . "-" . $tanggal_hari_ini . ".xlsx";
